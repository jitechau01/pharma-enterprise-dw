# Minimal VPC for MWAA: 2 public subnets (NAT gateways) + 2 private subnets
# (where the MWAA environment's ENIs actually live, network access mode
# PRIVATE_ONLY). This mirrors AWS's own MWAA quick-start VPC topology.
...
# Security group for MWAA's VPC endpoints / ENIs (self-referencing ingress
# only, per AWS's documented MWAA SG requirements).
resource "aws_security_group" "mwaa" {
  name_prefix = "${var.name_prefix}-mwaa-sg-"
  description = "MWAA environment security group"
  vpc_id      = aws_vpc.this.id
  tags        = merge(var.tags, { Name = "${var.name_prefix}-mwaa-sg" })
